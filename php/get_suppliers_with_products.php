...
try {
    ...
    foreach ($suppliers as $supplier) {
        ...
        $product_count = 0;
        try {
            $products_query = "SELECT id, name, price, description, image_name FROM products WHERE supplier_id = ?";
            $products_stmt = $db->prepare($products_query);
            $products_stmt->execute([$supplier['id']]);
            ...
            foreach ($products_data as $product) {
                $products[] = [
                    ...
                    'image' => $product['image_name'] ?: null,
                    'image_url' => $product['image_name'] ? '../uploads/products/' . $product['image_name'] : null
                ];
            }
        } catch (Exception $e) {
            $product_count = 0;
        ...
        try {
            $rating_query = "SELECT AVG(rating) as avg_rating, COUNT(*) as review_count FROM reviews WHERE supplier_id = ?";
    ...
    $response = [
        ...
        'version' => 'real_database_with_products'
    ];
